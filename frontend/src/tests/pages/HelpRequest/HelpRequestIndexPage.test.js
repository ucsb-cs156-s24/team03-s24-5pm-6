...
describe("HelpRequestIndexPage tests", () => {
    ...
    test("renders three help requests correctly for regular user", async () => {
        ...
        const requesterEmail = screen.getByText("mario@mushroomkingdom.gov");
        expect(requesterEmail).toBeInTheDocument();
        
        const teamId = screen.getByText("5pm-6");
        expect(teamId).toBeInTheDocument();

        const tableOrBreakoutRoom = screen.getByText("14");
        ...
        const explanation = screen.getByText("Mama mia");
